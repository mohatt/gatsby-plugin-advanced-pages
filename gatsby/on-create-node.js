...
module.exports = ({ node, actions, getNode, createNodeId, createContentDigest }) => {
  ...
  if(!page.template){
    if(!options.template){
      throw new Error(`Missing 'template' metadata at '${fileNode.relativePath}'. No default template is set in plugin options either.`)
    }
  ...
  if(typeof routes !== 'object'){
    throw new Error(`Expected 'routes' metadata in ${fileNode.relativePath} to be an Object (got ${typeof routes})`)
  }
